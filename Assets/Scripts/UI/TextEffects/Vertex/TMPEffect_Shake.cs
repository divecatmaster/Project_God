...
{
    /// <summary>
    /// Individual character jitter/shake effect.
    /// </summary>
    [ExecuteAlways]
    [RequireComponent(typeof(TMP_Text))]
    public class TMPEffect_Shake : MonoBehaviour
    {
        [Header("Shake Settings")]
        [SerializeField] private float strength = 2f;
        [SerializeField] private float speed = 20f;
        [SerializeField] private bool usePerCharacterSeed = true;
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < characterCount; i++)
            {
                ...
                float seed = usePerCharacterSeed ? i * 1.5f : 0f;
                float offsetX = (Mathf.PerlinNoise(time + seed, 0) - 0.5f) * strength;
                float offsetY = (Mathf.PerlinNoise(0, time + seed) - 0.5f) * strength;
                Vector3 translation = new Vector3(offsetX, offsetY, 0);
